...
def call_gpt(
    ...
    print(responses)
    cache[str((user_prompt, system_prompt))].extend(
        [get_summary(x.message.content) for x in responses.choices]
    )
...
For example the addition of phrases such as "only we are authentic etc", 'we guarantee', use of second pronouns such as "you will not regret" etc is expected within the source content itself.""".format(
            summary=summary
        ).strip()
    except:
