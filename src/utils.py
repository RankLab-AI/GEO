...
def impression_para_based(sentences, n=5, normalize=True, alpha=1.1, beta=1.5, gamma2=1 / math.e):
    ...
    return (
        [x / sum(final_scores) for x in final_scores]
        if normalize and sum(final_scores) != 0
        else final_scores
    )
...
def impression_uniqueness_detailed(sentences, query, n=5, normalize=True, idx=0):
    return impression_subjective_impression(
        sentences, query, n=n, normalize=normalize, idx=idx, metric="uniqueness_detailed"
    )
...
def impression_subjective_impression(
    ...
    def convert_to_number(x, min_val=1.0):
        try:
            return max(min(5, float(x)), min_val)
        except:
            return min_val
